...
```mermaid
flowchart TD
    Trigger[/Test Trigger: Affected Packages/] --> Evaluate
    Schedule[/Initial Scheduled Test Modules/] --> Evaluate

    Evaluate[For each scheduled Test Module] --> HasMetadata{"Has '# Package:'\nmetadata?"}

    HasMetadata -- No --> Include[Include Test Module, Fallback]
    HasMetadata -- Yes --> Extract[Extract required packages, e.g., 'vim']

    Extract --> Match{"Are extracted packages\n(or their dependencies) in 'Affected Packages'?"}
